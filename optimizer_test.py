...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size, 
                learning_rate= 0.1, optimizer='adam', activation='relu'):
        """
        neural netweork with mu,ltiple activation functions and optimizers
...
# ==================== Activation Functions ==================
    ...
    def backward(self, X, y):
        ...
        if self.activation_type == 'relu':
            ...
            dZ1 = np.dot(dZ2, self.W2.T) * delf.activate_derivative(self.A1, is_a=True)

        dW1 = (1/m) * np.dot(X.T, dZ1)
        db1 = (1/m) * np.sum(dZ1, axis=0, keepdims=True)

        #update weights with choosen optimizer
        self._update_weights(dW1, db1, dW2, db2)

# ==================OPTIMIZERS==================

    def _update_weights(self, dW1, db1, dW2, db2):
        """update weight using choosen opitmzer"""
    ...
    def _adam_update(self, dW1, db1, dW2, db2):
        ...
        """
        self.t += 1 #increment timestamp

        #updte biased first moemnt (momentum)
        ...
        #update biased secoond moment (variance)
        ...
        mb1_hat = self.vb1 / (1 - self.beta1 ** self.t)
        mW2_hat = self.vW2 / (1 - self.beta1 ** self.t)
        mb2_hat = self.vb2 / (1 - self.beta1 ** self.t)
